...
const QuizHeader = ({ title, highlight, description, size, type = 'quiz' , titleColor = 'text-primary'}: props) => {
  return (
    <div className={` 
        ${size === 'md' ? 'space-y-4 md:space-y-5' : 'space-y-[3px] md:space-y-2' }`}>
        <h1 className={`font-semibold ${type === 'result' ? titleColor : 'text-white'} ${size === 'md' ? 'text-4xl md:text-5xl ' : 'text-xl md:text-3xl'}`}>
        {size === 'md' ? (
        <>
          <span >{title}</span>
